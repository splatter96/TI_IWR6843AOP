...
class get_data:
    ...
    def __load_cfgs(self, cfg_path_):
        ...
        self._vel_max = (
            ...
            / (
                ...
                * (
                    startFreq * 1e9
                    + (freqSlopeConst * 1e12) * (adcStartTime * 1e-6 + ADCDuration / 2)
                )
                * PRI
            )
            / ntx
    ...
    def __parse_data(self, buffer):
        start = (
            buffer.index(self.__magicWord) + 28
        )  # not suited for official people counting binary
        (num_points, num_tlvs, num_subframes), i = self.unpack(
            buffer, i=start, amount=3, data_type="I"
        )
        ...
        if not self.cfg_mode:
            ...
            if tlv_type == 1:
                try:
                    for j in range(num_points):
                        (x, y, z, vel), i = self.unpack(
                            buffer, i, amount=4, data_type="f"
                        )
                        if self.__roi:
                            if (
                                y >= self.__roi["x_l"]
                                and y <= self.__roi["x_u"]
                                and x <= -self.__roi["y_l"]
                                and x >= -self.__roi["y_u"]
                                and z >= self.__roi["z_l"]
                                and z <= self.__roi["z_u"]
                            ):
            ...
            if tlv_type == 7:
                try:
                    for j in range(num_points):
                        (snr, noise), i = self.unpack(
                            buffer, i, amount=2, data_type="H"
                        )
                        if self.__roi:
            ...
            if num_tlvs > 2:
                ...
                for extra_tlv in range(num_tlvs - 2):
                    if extra_tlv == 0:
                        (tlv_type, tlv_length), i = self.unpack(
                            buffer, i, amount=2, data_type="I"
                        )
                        remnant -= 8
                    ...
                    try:
                        ...
                        if tlv_type == 8:
                            ...
                            if remnant >= tlv_length + (num_tlvs - 3) * (
                                8 + self._rangeDopplerSize * self._rangeFFTSize
                            ):
                                for j in range(
                                    self._rangeFFTSize * self._numVirtualAnt
                                ):
                                    (imag, real), i = self.unpack(
                                        buffer, i, amount=2, data_type="h"
                                    )
                                    RA_Mat_F[j] = imag
                                    RA_Mat_R[j] = real
                                ...
                                warnings.warn(
                                    "OUTPUT_MSG_AZIMUT_ELEVATION_STATIC_HEAT_MAP WRONG"
                                )
                        ###  tlv5  ####
                        elif tlv_type == 5:
                            if remnant >= tlv_length:
                                for j in range(
                                    self._rangeFFTSize * self._rangeDopplerSize
                                ):
                                    X, i = self.unpack(
                                        buffer, i, amount=1, data_type="h"
                                    )
                                    RD_Mat[j] = X
                                    remnant -= 2
                            else:
                                warnings.warn("OUTPUT_MSG_RANGE_DOPPLER_HEAT_MAP WRONG")
                        else:
                            warnings.warn(
                                "TLV {} not supported yet, waiting one more time...".format(
                                    tlv_type
                                )
                            )
                    except Exception as e:
        ...
        ## Using 3d_tracking cfgs
            ...
            if tlv_type == 1020:
                if not get_unit:
                    (
                        (elevation_unit, azimuth_unit, vel_unit, range_unit, snr_unit),
                        i,
                    ) = self.unpack(buffer, i, amount=5, data_type="f")
                    get_unit = True
                else:
                    i += 20
                try:
                    for j in range(num_points):
                        (elevation, azimuth), i = self.unpack(
                            buffer, i, amount=2, data_type="B"
                        )
                        (vel, d, snr), i = self.unpack(
                            buffer, i, amount=3, data_type="H"
                        )
                        if elevation >= 128:
                            elevation -= 256
                        if azimuth >= 128:
                        ...
                        x = (
                            d
                            * range_unit
                            * cos(elevation * elevation_unit)
                            * sin(azimuth * azimuth_unit)
                        )
                        y = (
                            d
                            * range_unit
                            * cos(elevation * elevation_unit)
                            * cos(azimuth * azimuth_unit)
                        )
                        z = d * range_unit * sin(elevation * elevation_unit)
                        if self.__roi:
                            if (
                                x >= self.__roi["x_l"]
                                and x <= self.__roi["x_u"]
                                and y >= self.__roi["y_l"]
                                and y <= self.__roi["y_u"]
                                and z >= self.__roi["z_l"]
                                and z <= self.__roi["z_u"]
                            ):
    ...
    def read(self):
        while time.time() < (self.__start_time + self.__timeout):
            ...
            # print(f"bffer {self.__buffer_temp}")
    ...
    def unpack(buffer, i, order="<", amount=1, data_type="I"):
        ...
        try:
            parsed_data = struct.unpack(
                order + str(amount) + data_type,
                buffer[i : (i + data_size[data_type] * amount)],
            )
