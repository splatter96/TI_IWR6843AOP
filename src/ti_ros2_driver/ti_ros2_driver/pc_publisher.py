...
class rospublisher(Node):
    def __init__(self, queue_size=100, max_depth=multiprocessing.cpu_count()):
        ...
        self.declare_parameter(
            "debug_log_path", os.getcwd() + "/src/ti_ros2_driver/debug/result.json"
        )
        self.cfg_path_ = (
            self.get_parameter("cfg_path").get_parameter_value().string_value
        )
        self.command_port_ = (
            self.get_parameter("command_port").get_parameter_value().string_value
        )
        self.data_port_ = (
        ...
        self.publish_tar = (
            self.get_parameter("publish_target_tracker")
            .get_parameter_value()
            .bool_value
        )
        self.plot_RA_ = (
            self.get_parameter("output_RA_heatmap").get_parameter_value().bool_value
        )
        self.plot_RD_ = (
            self.get_parameter("output_RD_heatmap").get_parameter_value().bool_value
        )
        self.debug_ = self.get_parameter("debug_mode").get_parameter_value().bool_value
        self.debug_log_path_ = (
            self.get_parameter("debug_log_path").get_parameter_value().string_value
        )
        roi = {}
        if xyz:
            ...
            if len(chs) != 3:
                self.get_logger().warn(
                    "Invalid roi bounds setting. No roi will be specified."
                )
            else:
                for ch_n, ch_v in enumerate(chs):
                    ...
                    if ch_n == 0:
                        roi.update({"x_l": ll, "x_u": ul})
                    if ch_n == 1:
                        roi.update({"y_l": ll, "y_u": ul})
                    if ch_n == 2:
                        roi.update({"z_l": ll, "z_u": ul})
        ...
        if self.publish_tar:
            self.target_publisher_ = self.create_publisher(
                PointCloud2, self.topic_ + "_target", qos_profile
            )

        self.heatmap_pub = self.create_publisher(
            Image, "/radar_heatmap", 10
        )  # , qos_profile)

        ## parse ti raw data in a producer-consumer mode
        self._pcbuffer = queue.Queue(queue_size)
        self.ti = get_data(
            command_port=self.command_port_,
            data_port=self.data_port_,
            cfg_path=self.cfg_path_,
            roi=roi,
        )
        if self.plot_RA_ or self.plot_RD_:
            self.heatmap = plotHM(
                if_ra=self.plot_RA_,
                if_rd=self.plot_RD_,
                hang=float(self.ti._ms_per_frame / 1000),
            )
        self.heatmap.init_hm(
            self.ti._rangeFFTSize,
            self.ti._rangeDopplerSize,
            self.ti._range_max,
            self.ti._vel_max,
            self.ti._range_resolution,
            self.ti._vel_abs_max,
            self.ti._numVirtualAnt - 4,
            400,
        )

        self.br = CvBridge()

        range_max = self.ti._range_max
        angle_bins = self.ti._numVirtualAnt - 4
        range_bins = self.ti._rangeFFTSize
        range_res = self.ti._range_resolution
        grid_res = 400
        ...
        while self.shut_down == 0:
            ...
            if empty_times > 20:
                ...
                self.get_logger().error(
                    "No data received for more than 20 times, queue released"
                )
                self.shut_down = 1
            if self.debug_:
                producer_timer = InstrumentationTimer()
            try:
    ...
    def __consumer(self):
        while self.shut_down == 0:
            if self.debug_:
                consumer_timer = InstrumentationTimer()
            self.cv.acquire()
            self.cv.wait()
            while True:
                try:
            ...
            if self.plot_RD_ or self.plot_RA_:
                self.processHeatMap(ti_data[1], ti_data[2], ti_data[3])
            if self.publish_tar:
                (tar_msg, num_tars) = self.processTargetTracker(ti_data[1], ti_data[2])
            if rclpy.ok():
                self.publisher_.publish(pcl_msg)
                if self.publish_tar:
                    self.target_publisher_.publish(tar_msg)
            else:
                return
            if self.debug_:
                self.get_logger().info("Publishing %s points" % num_points)
                if self.publish_tar:
                    self.get_logger().info("Detected %s points" % num_tars)
        ...
        if not self.heatmap.is_init:
            ...
            if self.ti.angle == 4:
                self.heatmap.init_hm(
                    ...
                    self.ti._range_resolution,
                    self.ti._vel_abs_max,
                    self.ti._numVirtualAnt,
                    400,
                )
        if self.plot_RD_:
            RD_Mat = np.reshape(
                RD_Mat, (self.ti._rangeFFTSize, self.ti._rangeDopplerSize)
            )
        if self.plot_RA_:
            ...
            RA_Mat = np.array(
                [
                    RA_Mat_T[i] + 1j * RA_Mat_F[i]
                    for i in range(self.ti._rangeFFTSize * self.ti._numVirtualAnt)
                ]
            )
            ...
            try:
                ...
                if self.ti.angle == 8:
                    RA_Mat = RA_Mat[:, 4:]  # Range-Azimuth FFT
                if self.ti.angle == 4:
                    RA_Mat = RA_Mat[:, :-4]  # Range-Elevation FFT
            except Exception as e:
                self.get_logger().error(
                    "Fail to perform fft for Range-Amizuth complex dtype\n" + e
                )

            ra_fft = np.fft.fftshift(RA_Mat, axes=(1,))
            ra_fft = ra_fft[:, 1:]
            ...
            data = zi[::-1, ::-1]
            # np.save("data_image_new.npy", zi[::-1, ::-1])
            ...
            # cv2.waitKey(1)
            # cv2.imwrite(f"img_{time.time()}.png", cv_img)
    ...
    def processTargetTracker(self, target, conv):
        ...
        tar_msg.fields = [
            ...
            PointField(name="z", offset=8, datatype=PointField.FLOAT32, count=1),
            PointField(
                name="velocity_x", offset=12, datatype=PointField.FLOAT32, count=1
            ),
            PointField(
                name="velocity_y", offset=16, datatype=PointField.FLOAT32, count=1
            ),
            PointField(
                name="velocity_z", offset=20, datatype=PointField.FLOAT32, count=1
            ),
            PointField(
                name="acceleration_x", offset=24, datatype=PointField.FLOAT32, count=1
            ),
            PointField(
                name="acceleration_y", offset=28, datatype=PointField.FLOAT32, count=1
            ),
            PointField(
                name="acceleration_z", offset=32, datatype=PointField.FLOAT32, count=1
            ),
        ]
