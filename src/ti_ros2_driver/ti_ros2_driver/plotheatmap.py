...
class plotHM:
    def __init__(self, if_ra, if_rd, hang=100):
        ...
        self.animation = animation.FuncAnimation(
            self.figure, self.update, interval=self.periodic
        )
    ...
    def init_hm(
        ...
        if self.if_ra_:
            t = np.array(range(-angle_bins // 2 + 1, angle_bins // 2)) * (
                2 / angle_bins
            )
            t = np.arcsin(t)
        ...
        if self.num_hm == 2:
            ...
            if self.if_rd_:
                self.plot_RD(
                    range_bins, doppler_bins, range_max, doppler_max, doppler_bias
                )
            if self.if_ra_:
                self.plot_RA(range_bins, range_max, grid_res)
        self.is_init = True
